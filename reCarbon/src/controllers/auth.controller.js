...
const login = async (req, res) => {
  try {
    ...
    const account = await CompanyAccount.findOne({ email: normalizedEmail });
    if (!account) {
    ...
    const token = jwt.sign(
      {
        ...
        manufacturingCompanyId: account.manufacturingCompanyId,
      },
    ...
    return res.status(200).json({
      ...
      token,
      companyId: account.manufacturingCompanyId,
    });
  } catch (error) {
